...
from lborg.data_helpers import add_dates_db
from lborg.db import create_query, query_database
from lborg.tables import make_table

import argparse
parser = argparse.ArgumentParser('Add dates to Database')
parser.add_argument('--db_name', type=str, default='data/dates.db', help='Database name')
...
def main():
    ...
    add_dates_db(args.json_name,db_name=args.db_name)
    # query the database (partial)
    query = create_query(args.db_name,'dates',columns=['date','hours'], order='date')
